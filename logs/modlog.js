...
module.exports = async (client) => {
  client.on("guildBanAdd", async (guild, user) => {
    ...
    if (data) {
      ...
      banEmbed.setColor(parseInt("0x000000", 16));
      banEmbed.setThumbnail(target.displayAvatarURL({ dynamic: true }));
      banEmbed.setAuthor(
        `${target.username} was banned`,
  ...
  client.on("guildBanRemove", async (guild, user) => {
    ...
    if (data) {
      ...
      unbanEmbed.setColor(parseInt("0x000000", 16));
      unbanEmbed.setThumbnail(target.displayAvatarURL({ dynamic: true }));
      unbanEmbed.setAuthor(
        `${target.username} was unbanned`,
        target.displayAvatarURL({ dynamic: true })
      );
