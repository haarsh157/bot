...
module.exports = async (client) => {
  client.on("guildMemberAdd", async (member) => {
    ...
    if (data) {
      ...
      memberAddEmbed.setAuthor(
        member.user.tag,
        member.user.displayAvatarURL({ dynamic: true })
      );
      memberAddEmbed.setThumbnail(
        member.user.displayAvatarURL({ dynamic: true })
      );
      memberAddEmbed.setColor(parseInt("0x000000", 16));
      memberAddEmbed.setDescription(
        `A new member has joined the server.\nAccount created at <t:${Math.round(
          member.user.createdTimestamp / 1000
        )}:R>\nMember joined at <t:${Math.round(
    ...
    if (data) {
      ...
      if (
        ...
        if (oldTopRole.id !== newTopRole.id) {
          roleUpdateEmbed.addFields({
            name: "Top Role Update",
            value: `Old Top Role: <@&${oldTopRole.id}>\n\nNew Top Role: <@&${newTopRole.id}>`,
          });
        }
