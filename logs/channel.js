...
module.exports = async (client) => {
  client.on("channelCreate", async (channel) => {
    ...
    if (data) {
      ...
      channelcreate.setThumbnail(
        `${executor.displayAvatarURL({ dynamic: true })}`
      );

      channelcreate.setColor(parseInt("0x000000", 16));
      channelcreate.setFooter(
        channel.guild.name,
  ...
  client.on("channelDelete", async (channel) => {
    ...
    if (data) {
      ...
      channeldelete.setThumbnail(
        `${executor.displayAvatarURL({ dynamic: true })}`
      );

      channeldelete.setColor(parseInt("0x000000", 16));
      channeldelete.setFooter(
        channel.guild.name,
        channel.guild.iconURL({ dynamic: true })
      );
  ...
  client.on("channelUpdate", async (o, n) => {
    ...
    if (data) {
      ...
      channelupdate.setAuthor(
        executor.tag,
        executor.displayAvatarURL({ dynamic: true })
      );
      channelupdate.setTimestamp();
      channelupdate.setDescription(
        `${o.type === "GUILD_TEXT" ? "Text" : "Voice"} Channel <#${
          n.id
        }> updated by ${executor}\n`
      );

      channelupdate.setColor(parseInt("0x000000", 16));
      if (oldName !== newName)
      ...
      if (oldNsfw !== newNsfw)
        channelupdate.addFields({
          name: "Channel NSFW State Updated",
          value: `\`${o.nsfw}\` -> \`${n.nsfw}\``,
          inline: false,
        });
      if (oldcooldown !== newcooldown)
        channelupdate.addFields({
          name: "Channel Slowmode Updated",
          value: `\`${o.rateLimitPerUser} seconds \` -> \`${n.rateLimitPerUser} seconds \``,
          inline: false,
        });
      if (oldTopic !== newTopic)
        channelupdate.addFields({
          name: "Channel Topic Updated",
          value: `\`${o.topic} \` -> \`${n.topic} \``,
          inline: false,
        });
      if (oldrtcRegion !== newrtcRegion)
        channelupdate.addFields({
          name: "Channel Voice Region Updated",
          value: `\`${o.rtcRegion} \` -> \`${n.rtcRegion} \``,
          inline: false,
        });
      if (oldbitrate !== newbitrate)
        channelupdate.addFields({
          name: "Channel Voice Bitrate Updated",
          value: `\`${oldbitratevalue} Kbps\` -> \`${newbitratevalue} Kbps\``,
          inline: false,
        });
      if (olduserLimit !== newuserLimit)
        channelupdate.addFields({
          name: "Channel Voice UserLimit Updated",
          value: `\`${o.userLimit} users\` -> \`${n.userLimit} users\``,
          inline: false,
        });
      else if (o.type === "GUILD_VOICE") {
        channelupdate.addFields({
          name: "Channel Voice Updated",
          value: `Channel Video Quality Mode Updated`,
          inline: false,
        });
      }
      channelupdate.addFields({
