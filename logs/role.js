...
module.exports = async (client) => {
  ...
  client.on("roleUpdate", async (o, n) => {
    ...
    if (data) {
      const rolename = new EmbedBuilder();
      rolename.setColor(parseInt("0x000000", 16));
      rolename.setAuthor(
        `Role Update`,
        executor.displayAvatarURL({ dynamic: true })
      );
      ...
      if (o.name !== n.name)
        rolename.addFields({
          name: "Name",
          value: `Old Name : \`${o.name}\` New Name : \`${n.name}\``,
          inline: false,
        });
      if (o.color !== n.color)
        rolename.addFields({
          name: "Color",
          value: `Old Color : \`${o.color}\` New Color : \`${n.color}\``,
          inline: false,
        });
      if (o.hoist !== n.hoist)
        rolename.addFields({
          name: "Display",
          value: `Old Display : \`${o.hoist}\` New Display : \`${n.hoist}\``,
          inline: false,
        });
      if (o.mentionable !== n.mentionable)
        rolename.addFields({
          name: "Mentionable",
          value: `Old Mentionable : \`${o.mentionable}\` New Mentionable : \`${n.mentionable}\``,
          inline: false,
        });
      const oldPermissions =
        o.permissions instanceof Permissions
          ? o.permissions
              .toArray()
      ...
      const newPermissions =
        n.permissions instanceof Permissions
          ? n.permissions
              .toArray()
              .map((permission) => `\`${permission}\``)
              .join(", ")
          : "No Permissions";
      if (n.permissions !== o.permissions)
        rolename.addFields({
          name: "Permissions",
          value: `Old Permissions : ${
            oldPermissions ? oldPermissions : "`No Permissions`"
          }\n\nNew Permissions : ${
            newPermissions ? newPermissions : "`No Permissions`"
          }`,
          inline: false,
        });
